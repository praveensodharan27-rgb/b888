...
export function useLocationPersistence() {
    ...
    const loadPersistedLocation = async () => {
      try {
        ...
        if (storedLocation) {
          ...
          // Only try to fetch from user profile if user is authenticated
          if (isAuthenticated) {
            try {
              ...
              // User not logged in, token expired, or no preferred location
              // Only log if it's not a 401 (expected when not authenticated)
              if (error?.response?.status !== 401) {
  ...
  }, [fetchLocationBySlug, saveToLocalStorage, saveToUserProfile, updateUrlWithLocation, removeLocationParamFromUrl, searchParams, isAuthenticated]); // Run when authentication status changes
